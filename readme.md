...
ROS1 与 ROS2 共用同一份代码，节点名与可执行文件名均为 `fm_driver`。构建时依据环境变量 `ROS_VERSION` 自动选择版本，无需手动指定。
...
话题均以节点名为命名空间，即 `/fm_driver/<话题名>`。消息字段定义见 [msg/](msg/) 目录。
...
一旦启用 ROS1 或 ROS2，后四个选项默认全部关闭：ROS 构建只产出 `fm_driver` 节点，不会连带编译上位机示例、单元测试，也不会去拉取 Catch2 / Foxglove SDK。如仍需其中某项，显式打开即可，例如在 ROS 工作空间里编译并运行单元测试：
